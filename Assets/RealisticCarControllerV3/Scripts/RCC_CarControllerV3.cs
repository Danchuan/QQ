...
public class RCC_CarControllerV3 : NetworkBehaviour {
	...
	// 设置车辆的车轮驱动器。 如果您使用的是rwd，则必须小心后轮对撞机
	...
	public float[] gearSpeed;		// Target Speed For Changing Gear.d
...
//		anyWheel.ConfigureVehicleSubsteps(10f, 2, 2);
	...
	public void CreateWheelColliders (){
		...
		WheelColliders.transform.rotation = Quaternion.identity;
	...
	void ApplyMotorTorque(RCC_WheelCollider wc, float torque)
	{
		...
		float tmpT = ((torque * (1 - clutchInput) * _boostInput) * _gasInput) * (engineTorqueCurve[currentGear].Evaluate(wc.wheelRPMToSpeed * direction) * direction);
		wc.wheelCollider.motorTorque = tmpT;
...
//			}
		...
		velocityDirection.rotation = Quaternion.Lerp(velocityDirection.rotation, Quaternion.AngleAxis(Mathf.Clamp(velocityAngle / 3f, -45f, 45f), Vector3.up), Time.fixedDeltaTime * 20f);
		steeringDirection.rotation = Quaternion.Euler (0f, FrontLeftWheelCollider.steerAngle, 0f);

		int normalizer = 1;

		if (steeringDirection.rotation.y > velocityDirection.rotation.y)
			normalizer = 1;
		else
			normalizer = -1;

		float angle2 = Quaternion.Angle (velocityDirection.rotation, steeringDirection.rotation) * (normalizer);
